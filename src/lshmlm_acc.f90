...
!***********************************************************************
!
      subroutine lshmlm_acc(x,y,nmes,xll,it,nsub,beta,nest,nilev,quad, &
...
!     case
      ...
      type (accurate) theta(maxpar),qprob(2,256),e2,r,rr,e8(maxpar),b, &
             ...
             d2(maxpar*(maxpar+1)/2),d1(maxpar),dd1(maxpar), &
             qloc(2,256),e7(maxpar),der1(maxpar),dpieta,e3s,rs,e1s, &
             e1ss,rrs,rrss,e8s(maxpar),qpe2,qpxl,li2star,xl1, &
             a1(maxpar),a2(maxpar*(maxpar+1)/2),a3(maxpar*(maxpar+1)/2), &
             dd2(maxpar*(maxpar+1)/2)
...
!$OMP SHARED(scores,sflag,xil,sign), &
