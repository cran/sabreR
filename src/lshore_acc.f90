...
!***********************************************************************
!
      subroutine lshore_acc(x,y,nmes,xll,it,nsub,beta,nest,nilev,quad, &
...
!-----------------------------------------------------------------------
      ...
      integer nmes,nest,ncov,maxcol,nsub(2),it(2,nsub(1)),flag,nilev, &
              iter,cflag,ncat(2),offpos(3),nm(3),ind,sign,iquad,maxcat
...
!     case
      ...
      type (accurate) theta(maxpar),e2,r,rr,b,pi,bb,meansc(maxpar), &
             ...
             qprob(256),qpe2,e1,e3,e4(maxcat),e7(maxpar),e9(maxcat), &
...
!$OMP SHARED(firstcase,lastcase,i1,i2,ind,this_processor,link,flag,c), &
!$OMP SHARED(num_processors,nest,ncov,nm,iquad,quad,mus,taus,nilev,it), &
!$OMP SHARED(ncut,ncat,rowind,theta,x,offlag,offpos,y,sflag,xil,sign), &
