...
!
      program sabre
!     version: 04.06.08, 14:04
!-----------------------------------------------------------------------
      implicit none
!-----------------------------------------------------------------------
      character(len=80) :: input_file, arg_value
      character(len=20) :: nextarg
      character(len=80) :: outbuf
      integer equals_position, ivalue
      integer iarg, iargc, versionflag, terseflag, inputflag
      integer length
      logical error, stop
      integer ierror
      integer mxx_size, maxy_size, maxvar_size, maxpar_size
!-----------------------------------------------------------------------
!     Main program calls mpi_init, processes any arguments and then
!     calls the remainder of Sabre to try
!     to avoid unnecessary memory allocation in MPI master process
!-----------------------------------------------------------------------
!---- Initialise MPI
!-----*************
...
!     = sign and then the value with no separating spaces in which case
!     only convert the first part (up to the = sign)
!
...
!
      if (nextarg(1:7) == 'version' .or. nextarg(1:8) == '-version' &
      .or. nextarg(1:8) == '/version') then
!
         versionflag = 1
!
      else if (nextarg(1:5) == 'terse' .or. nextarg(1:6) == '-terse' &
      .or. nextarg(1:6) == '/terse') then
          terseflag = 1
      else if (nextarg(1:5) == 'input' .or. nextarg(1:6) == '-input' &
      .or. nextarg(1:6) == '/input') then
!
         inputflag = 1
!
...
!
!
      else if (nextarg(1:3) == 'dat' .or. nextarg(1:4) == '-dat' &
      .or. nextarg(1:4) == '/dat') then
!
!---------******************
...
!
      else if (nextarg(1:3) == 'obs' .or. nextarg(1:4) == '-obs' &
      .or. nextarg(1:4) == '/obs') then
!
!---------******************
          call get_arg_value(iarg,nextarg,arg_value)
!---------******************
...
!
      else if (nextarg(1:3) == 'var' .or. nextarg(1:4) == '-var' &
      .or. nextarg(1:4) == '/var') then
!
!---------******************
          call get_arg_value(iarg,nextarg,arg_value)
!---------******************
...
!
      else if (nextarg(1:3) == 'par' .or. nextarg(1:4) == '-par' &
      .or. nextarg(1:4) == '/par') then
!
!---------******************
          call get_arg_value(iarg,nextarg,arg_value)
!---------******************
...
!
    ...
    2 continue
!
!---- Call remainder of Sabre "main program"
!-----***************
!
!    Initialise abnormal stop flag
!
      stop = .false.
!
      call sabre_main( stop, mxx_size, maxy_size, maxvar_size, maxpar_size, &
                       versionflag, terseflag, inputflag, input_file )
!-----***************
!
      stop
!
...
!     There may be an equals sign separating the two, with or without
!     spaces
!-----------------------------------------------------------------------
...
!             Equals sign by itself found so next argument should be
!             value
              iarg = iarg + 1
!
              if (iarg > iargc()) then
...
!             No equals sign prefix so we have just read the argument
!             value, i.e. nothing more to do
          end if
!
      end if
!
