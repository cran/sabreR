...
!***********************************************************************
!
      subroutine lsbore(x,y,nmes,xll,it,nsub,beta,nest,nilev,quad,nm, &
                        score,hess,ncov,flag,ifail,maxcol,link,risk, &
                        corr,iter,cflag,step,scnorm,scores,n1lev,family, &
                        ind,xil,sign,iquad,aquad,mus,taus,offlag,offpos, &
                        ncat,maxcat)
...
!-----------------------------------------------------------------------
      ...
      integer nmes,nest,ncov,maxcol,nsub(2),nm(3),it(2,nsub(1)),flag, &
              nilev,risk(nmes),iter,cflag,n1lev,ind,sign,iquad, &
...
!     case
      ...
      character(len=5) :: str
      double precision theta(maxpar),thetanir,qprob(2,256),e2,r,e2s(2), &
             b,pi,pil(nmes),bb,d,s1,s2,meansc(maxpar), &
             rd(maxcat),sumxb(nmes),e3(2),expme,deri(maxpar),e,phi, &
             listar,d1(maxpar),qloc(2,256),e7(2,maxpar),der1(maxpar), &
             dpieta,rs,e3s(2),qpe2,e2s2,phi1(0:maxcat), &
             dpieta1(0:maxcat),e4(2,maxcat)
      double precision hesinc(maxpar*(maxpar+1)/2),score_total(maxpar), &
             xll_total
      integer t,i,irow,l,j1,k,m,j2,rowind(nsub(1)),ir,i1,i2,k1,k2, &
              ncut(3),c(2),cat,jj,n,nic1,nic2
...
!$OMP SHARED(firstcase,lastcase,i1,i2,ind,this_processor,nic1,nic2), &
...
!$OMP SHARED(z1,z2,sflag,xil,sign,c,risk,n1lev,corr,nmes,ncut,ncat), &
...
!
                      if (ir == 1) then
                          b = sumxb(irow) + theta(nic2+1)*qloc(1,j1)
                      else if (.not. corr) then
                          b = sumxb(irow) + theta(nic2+2)*qloc(2,j2)
                      else
                          b = sumxb(irow) + theta(nic2+2)* &
                          (theta(nic2+3)*qloc(1,j1) + &
                          sqrt(1 - theta(nic2+3)**2)*qloc(2,j2))
                      end if
!
                      e = fexp(b,iflow)
!
...
!
                      do 398 k = 1,ncut(1)
                          d1(nilev+k) = d1(nilev+k) + c(1)*qpe2*e4(1,k)
  398                 end do
!
                      do 399 k = 1,ncut(2)
                          d1(nic1+k) = d1(nic1+k) + c(2)*qpe2*e4(2,k)
  399                 end do
!
!-------------------- sigma_1 = phi_1
                      d1(nic2+1) = d1(nic2+1) + &
